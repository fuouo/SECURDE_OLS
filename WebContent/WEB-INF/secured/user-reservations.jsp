  ...
  <div class="row">
    ...
    <div class="col-sm-9 col-lg-10 content admin-area">
        ...
        <div class="panel panel-default">
          ...
          <div id="current-rm" class="panel-collapse collapse in">
            <div class="panel-body">
            
            <c:choose>
            	<c:when test="${currentRMSize == '0'}">
            		<h3>No Reserved Books at the moment</h3>
            	</c:when>
            </c:choose>
              <c:forEach items="${current_reserved_rm}" var="rm">
              	<div id = "${rm.RMID_Location}" class="log-item bordered-dark">
	                ...
	                Return on : <span class="date_returned">${rm.dateReturned}</span>
					<br>
              	</div>
              </c:forEach>
            </div>
        ...
        <div class="panel panel-default">
          <div class="panel-heading">
            <a data-toggle="collapse" href="#current-book-log">Previous Reading Material Reservations</a>
          </div>
          ...
          <div id="current-rm" class="panel-collapse collapse in">
            ...
            <c:choose>
            	<c:when test="${previousRMSize == '0'}">
            		<h3>No Previously Reserved Reading Materials yet</h3>
            	</c:when>
            </c:choose>
              <c:forEach items="${previous_reserved_rm}" var="rm">
              	<div id = "${rm.RMID_Location}" class="log-item bordered-dark">
	                <a class="title">${rm.title}</a>
	                <div class="status reserved-status">Reserved</div>
	                Reserved on : <span class="date_reserved">${rm.dateReserved}</span><br>
